...
* CTRL-Shift-E: show random sketch from the gallery
* CTRL-Shift-X: make random change to the current sketch
* CTRL-Shift-Y: toggle the VJ control panel (a hardware-style deck showing the current sketch as editable channel strips: drag faders to change parameters live or click any value readout to type an exact number, use the patch points between modules to insert functions, drag module headers to reorder, bypass a function from its chip menu to mute it without deleting it, and the ⏏ button to pop the deck out into its own tab — the pop-out is the self-contained deck page, so it survives reloads and its URL works on any device on the network (when no relay is available it falls back to a lightweight same-window popup) — or the picture-in-picture button, where supported, to float the deck above other apps while the visuals keep running; the QR button next to ⏏ opens the pairing page for tablets/phones). The deck also has: a scene bank (click an empty slot or shift+1-8 to save the current sketch with a thumbnail, click or press 1-8 to recall, right-click a slot for clear/MIDI-pad options, plus JSON export/import of the whole bank), MIDI-learn (right-click any fader, move a controller knob — mappings persist and the range is editable; scene slots learn note pads the same way), audio- and mouse-reactive binding (right-click a fader → bind to audio or mouse; the parameter becomes a boxed group with an input picker plus SCALE and OFFSET rows — the input runs 0..1, so the value is input × scale + offset), step-sequencer editing for array arguments including fast/offset/smooth/ease modifiers, and editable rows for external sources (camera / screen / video / image on s0–s3). The scene bank can auto-cycle through its saved slots (play button next to the bank; right-click it to set the pace), the SPEED row writes a `speed = …` line into the sketch so every saved scene carries its own tempo (the row is always shown — touching it adds the line if the sketch has none), right-clicking the FFT button adds audio response settings (`a.setSmooth` / `setScale` / `setBins` / `setCutoff`) to the sketch as fader rows, the CODE button hides the code overlay for a clean stage view while the deck stays up (lit = code visible), undo/redo buttons next to HUSH step back and forth through every deck and code change (ctrl+z / ctrl+shift+z while the deck has focus), shuffle and dice buttons next to HUSH show a random example sketch or make a random change to the current one (the dice tweaks one value and pulses the row it hit; shift-click or right-click the dice to swap a whole function instead — either way a single undo press brings the previous sketch back), and in a pop-out or picture-in-picture deck the ◉ LIVE button streams a live preview of the visuals into that window (the preview pauses whenever the browser puts the main hydra tab to sleep, e.g. while it is fully hidden).
* Remote deck: the same VJ deck can run on **another device** — a tablet or laptop controlling hydra on the machine that drives the projector. Open `/deck.html` on the renderer's browser for a pairing QR code, scan it on the controller, done. Works offline on a LAN (plain http, everything vendored) and over the internet through an HTTPS deployment (outbound-only `wss://`, no port forwarding); the deck gets the full control surface plus a ◉ LIVE preview (WebRTC with an automatic relayed-JPEG fallback), and survives drops on either side with auto-reconnect and resync. Needs the relay sidecar from `compose.yaml` (the dev server carries it built-in). Full guide: [docs/remote-deck.md](docs/remote-deck.md).
...
npm run dev
```
The dev server also carries the remote-deck relay on `/ws`, so `http://localhost:5173/deck.html` works out of the box. For production, `compose.yaml` deploys the app plus the relay sidecar (`docker compose up -d`); the app boots fully offline — all runtime assets are vendored.
